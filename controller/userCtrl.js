...
const getaUser = asyncHandler(async (req, res) => {
    const { id } = req.params;
    validateMongoDbId(id);     //--- validation covered in user's crud
    try {
...
try{
    let products = [];
const user = await User.findById(_id);
//check if user has products in thier cart
...
const createOrder = asyncHandler(async(req, res)=>{
    ...
    try{
        ...
        let newOrder = await new Order({ // check
...
const updateOrderStatus = asyncHandler(async (req, res) =>{
    ...
    validateMongoDbId(id);
   try{
    const updateOrderSatus = await Order.findByIdAndUpdate(
        id, 
        {
        orderStatus: status,
        paymentIntent: {
